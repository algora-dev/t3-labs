export const INTERNAL_PRICING_VERSION = "2026-09-v0.1";
...
export const INTERNAL_PRICING = {
  ...
  catalogueSize: [
    ...
    option("31-100", "31-100 products", "Larger product catalogue and upkeep allowance.", 200, 20),
    option("101-250", "101-250 products", "Medium catalogue with added setup and support exposure.", 400, 40),
    option("251-500", "251-500 products", "Larger catalogue with more validation and upkeep.", 650, 65),
    option("501-1000", "501-1,000 products", "Large catalogue with higher ongoing support allowance.", 900, 100),
    option("1001-5000", "1,001-5,000 products", "Very large catalogue requiring more setup and platform support.", 1500, 175),
    option("5000+", "5,000+ products", "Enterprise-scale catalogue. Use as a ballpark before custom review.", 2500, 300),
  ],
  dataReadiness: [
    option("clean", "Clean structured data", "Products/prices supplied in a clean spreadsheet or structured source.", 0),
    option("minor", "Minor cleanup", "Some formatting, mapping or cleanup required before setup.", 200),
    option("mixed", "Multiple / mixed sources", "Information needs combining across spreadsheets, PDFs or other sources.", 500),
    option("significant", "Significant preparation", "Substantial extraction, cleanup or restructuring required.", 1000),
  ],
  pricingLogic: [
    option("simple", "Simple", "Mostly product x quantity with straightforward rules.", 0, 0),
    option("standard", "Standard rules", "Waste, packaging, rounding or several common business rules.", 250, 20),
    option("advanced", "Advanced", "Multiple dependencies, product relationships or layered calculation logic.", 500, 40),
    option("complex", "Complex", "Business-specific pricing logic, multiple scenarios or more involved calculations.", 1000, 75),
    option("bespoke", "Highly bespoke", "Extensive custom calculation logic or unusual commercial rules.", 2000, 125),
  ],
  takeoffComplexity: [
    option("standard", "Standard takeoff", "Core areas and standard lineal components.", 0, 0),
    option("extended", "Extended components", "Additional measurement types or custom components.", 250, 20),
    option("advanced", "Advanced takeoff", "More specialised measurement logic or workflows.", 500, 40),
    option("specialist", "Specialist / multi-stage", "Highly specific takeoff behaviour or multiple measurement stages.", 1000, 75),
  ],
  assistantScope: [
    option("basic", "Q&A + handoff", "Approved answers, basic product guidance and qualified human handoff.", 0, 0),
    option("guidance", "Sales guidance", "Recommendations, clarification questions and stronger buying guidance.", 300, 25),
    option("pricing", "Pricing + estimates", "Assistant can work through pricing inputs and return useful estimates.", 500, 50),
    option("advanced", "Advanced multi-step sales flow", "Longer guided conversations, multiple product decisions and connected tools.", 1000, 100),
    option("cross-system", "Cross-system assistant", "Advanced assistant connected to several tools, account states or workflows.", 2000, 150),
  ],
  tradeAccess: [
    option("none", "No trade / customer accounts", "Public or staff-only use.", 0, 0),
    option("basic", "Basic trade accounts", "Approved logins and trade pricing access.", 750, 75),
    option("tiers", "Trade tiers + saved jobs", "Multiple pricing tiers and repeat-user workflow.", 1000, 100),
    option("customer-pricing", "Customer-specific pricing", "Individual pricing, account rules and richer saved activity.", 1500, 150),
    option("portal", "Advanced trade portal", "Broader account functionality, permissions and repeat-order workflows.", 2500, 225),
  ],
  adminLevel: [
    option("none", "No customer admin", "T3 manages configuration changes.", 0, 0),
    option("basic", "Basic admin", "Manage products and standard prices.", 500, 50),
    option("advanced", "Advanced admin", "Products, pricing, users, trade controls and more settings.", 1000, 100),
    option("operations", "Operations workspace", "A broader internal workspace for jobs, staff actions and operational controls.", 1500, 150),
  ],
  analyticsLevel: [
    option("none", "No analytics layer", "Standard technical logging only.", 0, 0),
    option("basic", "Basic tracking", "Quote activity, usage and useful event tracking.", 500, 50),
    option("advanced", "Analytics + follow-up intelligence", "Commercial dashboards, product demand and follow-up opportunities.", 1000, 100),
  ],
  outputLevel: [
    option("screen", "On-screen result", "Standard result/output experience.", 0, 0),
    option("branded", "Branded email / PDF", "A branded customer-facing result sent or downloaded.", 250, 10),
    option("quote-pack", "Formal quote pack", "More complete quote-ready documents and outputs.", 500, 25),
    option("advanced", "Advanced generated outputs", "Several output types, richer documents or custom generation rules.", 1000, 50),
  ],
  integrationLevel: [
    option("none", "No external integration", "Standalone T3 workflow.", 0, 0),
    option("simple", "Simple webhook / email workflow", "Basic data handoff to an external destination.", 250, 20),
    option("standard", "One standard integration", "Connect one common CRM, accounting or business platform.", 750, 75),
    option("multiple", "Multiple / custom integrations", "Several systems or a more bespoke API connection.", 1500, 150),
    option("complex", "Complex integration layer", "High-complexity integration work or multiple critical systems.", 3000, 250),
  ],
  customisationLevel: [
    option("standard", "Standard branded framework", "T3 framework styled to the business brand.", 0, 0),
    option("minor", "Minor customisation", "Small layout, interaction or styling changes.", 250, 0),
    option("moderate", "Moderate bespoke UX", "Noticeably tailored screens, flow and interface behaviour.", 750, 0),
    option("bespoke", "Bespoke interface", "A substantially custom interface and customer journey.", 1500, 0),
    option("high", "Highly bespoke product", "Extensive custom UI/UX beyond the standard framework.", 3000, 0),
  ],
  customDevelopment: [
    option("none", "No additional custom development", "Covered by selected modules.", 0, 0),
    option("small", "Small custom requirement", "A focused piece of additional custom functionality.", 500, 0),
    option("medium", "Medium custom requirement", "A meaningful extra workflow or capability.", 1500, 50),
    option("large", "Large custom requirement", "A substantial custom workflow or internal system extension.", 3000, 100),
    option("major", "Major bespoke system work", "Large bespoke development component. Treat as a starting ballpark only.", 5000, 200),
  ],
  supportLevel: [
    option("self", "Self-managed", "Customer manages routine content/pricing changes where admin tools allow it.", 0, 0),
    option("standard", "Standard managed support", "Routine support and a modest allowance for ongoing assistance.", 0, 99),
    option("managed", "Managed", "More frequent help, changes and hands-on T3 involvement.", 0, 199),
    option("priority", "Priority managed", "Higher-touch support for more complex or business-critical implementations.", 0, 399),
  ],
} as const;
