        ...
        <main className="mt-6 grid gap-6">
          ...
          <div>
            <section className="rounded-xl border border-[#e7e9ef] bg-white p-5 sm:p-6">
              ...
              <div className="relative mt-4 min-h-[300px] sm:min-h-[470px]">
                <figure className="h-[320px] overflow-hidden rounded-lg border border-[#dfe2e9] bg-[#111318] shadow-md sm:mr-24 sm:h-[430px] lg:h-[500px]"><Image src={proposal.conceptImages.desktopHero.src} alt={proposal.conceptImages.desktopHero.alt} width={1440} height={960} className="h-full w-full object-cover object-top" /></figure>
                <figure className="absolute bottom-5 right-0 h-[245px] w-[116px] overflow-hidden rounded-[1.25rem] border-[4px] border-[#111318] bg-white shadow-xl sm:h-[365px] sm:w-[170px]"><Image src={proposal.conceptImages.mobileHero.src} alt={proposal.conceptImages.mobileHero.alt} width={390} height={844} className="h-full w-full object-cover object-top" /><figcaption className="sr-only">Mobile concept view</figcaption></figure>
              </div>
            </section>

          </div>

  {proposal.comparison ? <section className="rounded-xl border border-[#e7e9ef] bg-white p-6 shadow-[0_10px_32px_rgba(24,31,51,0.05)] sm:p-8" onMouseEnter={() => track(proposal, "comparison_viewed")}><div className="max-w-2xl"><p className="text-[11px] font-semibold uppercase tracking-[0.18em] text-[#758300]">Website comparison</p><h2 className="proposal-section-title mt-2">{copy.comparisonHeading ?? "Current website and proposed concept"}</h2><p className="proposal-body mt-3">{copy.comparisonIntro ?? `A fair side-by-side view of the current public website and the private concept created for ${proposal.companyName}.`}</p></div><div className="mt-6 grid gap-6 lg:grid-cols-2"><article className="rounded-xl border border-[#e1e4ea] bg-[#f8f9fb] p-4 sm:p-5"><div className="flex items-center justify-between gap-3"><p className="flex items-center gap-2 text-sm font-medium"><span className="h-3 w-3 rounded-full bg-[#aeb3bd]" />{proposal.comparison.currentLabel ?? "Current website"}</p><span className="text-[10px] font-medium uppercase tracking-[0.12em] text-[#777d89]">{proposal.comparison.currentBadge ?? "As captured"}</span></div>{proposal.comparison.currentSiteImage ? <Image src={proposal.comparison.currentSiteImage.src} alt={proposal.comparison.currentSiteImage.alt} width={1440} height={960} className="mt-4 aspect-[16/10] w-full rounded-lg border border-[#dfe2e9] object-cover object-top shadow-sm" /> : <div className="mt-4 grid aspect-[16/10] place-items-center rounded-lg border border-dashed border-[#cfd3dc] bg-white px-3 text-center text-[10px] font-medium text-[#777d89]">Current-site screenshot pending verified input</div>}<ul className="mt-5 grid gap-3">{proposal.comparison.currentPoints.map((point) => <li key={point} className="flex gap-3 text-[15px] leading-6 text-[#606575]"><Check muted />{point}</li>)}</ul></article><article className="relative overflow-hidden rounded-xl border border-[#d8e6a2] bg-[#fcfff4] p-4 sm:p-5"><div className="flex items-center justify-between gap-3"><p className="flex items-center gap-2 text-sm font-medium"><span className="h-3 w-3 rounded-full bg-[#d7ff00]" />{proposal.comparison.proposedLabel ?? "New website concept"}</p><span className="text-[10px] font-medium uppercase tracking-[0.12em] text-[#758300]">{proposal.comparison.proposedBadge ?? "Private concept"}</span></div><Image src={proposal.comparison.proposedImage.src} alt={proposal.comparison.proposedImage.alt} width={1440} height={960} className="mt-4 aspect-[16/10] w-full rounded-lg border border-[#dfe2e9] object-cover object-top shadow-sm" />{proposal.comparison.proposedSupportingImages?.length ? <div className="pointer-events-none absolute right-4 top-28 z-10 hidden w-[30%] gap-2 lg:grid">{proposal.comparison.proposedSupportingImages.map((image) => <figure key={image.src} className="rounded-md bg-[#07090d] p-1 shadow-[0_14px_32px_rgba(10,11,16,0.32)]"><Image src={image.src} alt={image.alt} width={1440} height={1668} className={`${image.presentation === "natural" ? "h-auto" : "aspect-[3/5] object-cover object-top"} w-full rounded border border-white/10`} /></figure>)}</div> : null}<ul className="mt-5 grid gap-3">{proposal.comparison.proposedPoints.map((point, index) => <li key={point} className={`flex gap-3 text-[15px] leading-6 text-[#606575] ${index === 2 ? "mt-4" : ""}`}><Check />{point}</li>)}</ul></article></div></section> : null}

          {proposal.conceptImages.supporting?.length ? <section className="rounded-xl border border-[#e7e9ef] bg-white p-5 sm:p-6" onMouseEnter={() => track(proposal, "concept_gallery_viewed")}><div className="flex flex-col gap-1 sm:flex-row sm:items-end sm:justify-between"><div><p className="text-[11px] font-semibold uppercase tracking-[0.18em] text-[#758300]">Concept views</p><h2 className="proposal-section-title mt-1">{copy.supportingViewsHeading ?? "Designed for useful project enquiries"}</h2></div><p className="text-sm text-[#606575]">{copy.supportingViewsNote ?? "Private concept views, not a public website."}</p></div><div className="mt-4 grid gap-4 sm:grid-cols-2">{proposal.conceptImages.supporting.map((image) => <figure key={image.src} className="aspect-[10/6] overflow-hidden rounded-lg border border-[#dfe2e9] bg-[#f7f8fa] shadow-sm"><Image src={image.src} alt={image.alt} width={1440} height={960} className="h-full w-full object-cover object-top" /></figure>)}</div></section> : null}

          {proposal.strengths?.length ? <section className="rounded-xl border border-[#dfe8bb] bg-[#fcfff4] p-5 shadow-[0_10px_32px_rgba(24,31,51,0.04)] sm:p-7"><div className="max-w-2xl"><p className="text-[11px] font-semibold uppercase tracking-[0.18em] text-[#758300]">A strong starting point</p><h2 className="proposal-section-title mt-2">What {proposal.companyName} is already doing well</h2><p className="proposal-body mt-3">The new concept builds on strengths the business already has.</p></div><div className="mt-6 grid gap-4 md:grid-cols-3">{proposal.strengths.map((strength) => <article key={strength.title} className="rounded-lg border border-[#dfe8bb] bg-white p-5"><Check /><h3 className="mt-4 text-[15px] font-medium leading-5">{strength.title}</h3><p className="mt-2 text-[15px] leading-6 text-[#606575]">{strength.description}</p></article>)}</div></section> : null}
          ...
          {proposal.brandDirection?.enabled ? <section className="overflow-hidden rounded-xl border border-[#e1e4ea] bg-[#0f1115] p-6 text-white shadow-[0_10px_32px_rgba(24,31,51,0.08)] sm:p-8"><div className="grid gap-6 lg:grid-cols-[0.95fr_1.05fr] lg:items-center"><div><p className="text-[11px] font-semibold uppercase tracking-[0.18em] text-[#d7ff00]">Mini brand direction</p><h2 className="proposal-section-title mt-2">{proposal.brandDirection.heading ?? "An optional new look for the business"}</h2><p className="mt-4 max-w-2xl text-base leading-7 text-white/70">{proposal.brandDirection.copy}</p></div>{proposal.brandDirection.images?.length ? <div className="grid gap-3 sm:grid-cols-2">{proposal.brandDirection.images.map((image) => <figure key={image.src} className="grid min-h-40 place-items-center rounded-lg border border-white/12 bg-black p-5"><Image src={image.src} alt={image.alt} width={520} height={180} className="h-auto max-h-28 w-full object-contain" /></figure>)}</div> : null}</div></section> : null}
