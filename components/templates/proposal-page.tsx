...
type ProposalEvent = "proposal_opened" | "video_started" | "video_25_watched" | "video_50_watched" | "video_90_watched" | "concept_gallery_viewed" | "comparison_viewed" | "offer_viewed" | "launch_interest" | "launch_email_requested" | "call_requested" | "interactive_preview_requested" | "removal_requested" | "do_not_contact_requested";
...
export function ProposalPage({ proposal }: { proposal: ProposalConfig }) {
  ...
  const openLaunch = () => {
    track(proposal, "launch_interest");
  ...
  const saveRemoval = (choice: "remove" | "suppress") => {
    track(proposal, choice === "suppress" ? "do_not_contact_requested" : "removal_requested");
    ...
    <div className="proposal-page min-h-screen bg-[radial-gradient(circle_at_85%_6%,rgba(215,255,0,0.07),transparent_20rem),#fbfcff] text-[#0a0b10]">
      ...
      <div className="mx-auto w-[min(1180px,calc(100%-28px))] py-4 sm:w-[min(1180px,calc(100%-40px))] sm:py-6">
        <header className="flex min-h-16 items-center justify-between gap-4 rounded-xl border border-[#e7e9ef] bg-white px-4 shadow-[0_8px_28px_rgba(24,31,51,0.06)] sm:px-6">
          ...
          <a href={proposal.actions.calendlyUrl} className="hidden text-sm font-semibold text-[#252933] hover:underline sm:block" onClick={() => track(proposal, "call_requested")}>Book a call</a>
        </header>
        ...
        <main className="mt-6 grid gap-6">
          <section className="grid gap-7 px-1 py-5 lg:grid-cols-[0.78fr_1.22fr] lg:items-start lg:px-4">
            <div>
              ...
              <div className="mt-3 h-1 w-8 rounded-full bg-[#d7ff00]" />
              <p className="mt-7 max-w-xl text-[clamp(1.05rem,2vw,1.2rem)] leading-[1.65] text-[#3e4352]">{proposal.hero.supportingCopy}</p>
              <div className="mt-8 flex flex-col gap-3 sm:flex-row"><Button href="#walkthrough">{"\u25b6"} Watch your walkthrough</Button><Button href="#package" secondary onClick={() => track(proposal, "offer_viewed")}>View the package</Button></div>
          ...
          <div>
            <section className="rounded-xl border border-[#e7e9ef] bg-white p-5 sm:p-6">
              <h2 className="proposal-section-title">See the concept in detail</h2>
              <div className="relative mt-4 min-h-[290px] sm:min-h-[390px]">
                <figure className="overflow-hidden rounded-lg border border-[#dfe2e9] bg-[#111318] shadow-md sm:mr-20"><Image src={proposal.conceptImages.desktopHero.src} alt={proposal.conceptImages.desktopHero.alt} width={1440} height={960} className="h-auto w-full" /></figure>
                <figure className="absolute bottom-0 right-0 w-[32%] max-w-[170px] overflow-hidden rounded-[1.4rem] border-[7px] border-[#111318] bg-white shadow-xl"><Image src={proposal.conceptImages.mobileHero.src} alt={proposal.conceptImages.mobileHero.alt} width={390} height={844} className="h-auto w-full" /><figcaption className="sr-only">Mobile concept view</figcaption></figure>
              </div>
            </section>

          </div>

          {proposal.comparison ? <section className="rounded-xl border border-[#e7e9ef] bg-white p-6 shadow-[0_10px_32px_rgba(24,31,51,0.05)] sm:p-8" onMouseEnter={() => track(proposal, "comparison_viewed")}><div className="max-w-2xl"><p className="text-[11px] font-semibold uppercase tracking-[0.18em] text-[#758300]">Website comparison</p><h2 className="proposal-section-title mt-2">Current website and proposed concept</h2><p className="proposal-body mt-3">A fair side-by-side view of the current public website and the private concept created for {proposal.companyName}.</p></div><div className="mt-6 grid gap-6 lg:grid-cols-2"><article className="rounded-xl border border-[#e1e4ea] bg-[#f8f9fb] p-4 sm:p-5"><div className="flex items-center justify-between gap-3"><p className="flex items-center gap-2 text-sm font-medium"><span className="h-3 w-3 rounded-full bg-[#aeb3bd]" />Current website</p><span className="text-[10px] font-medium uppercase tracking-[0.12em] text-[#777d89]">As captured</span></div>{proposal.comparison.currentSiteImage ? <Image src={proposal.comparison.currentSiteImage.src} alt={proposal.comparison.currentSiteImage.alt} width={1440} height={960} className="mt-4 aspect-[16/10] w-full rounded-lg border border-[#dfe2e9] object-cover object-top shadow-sm" /> : <div className="mt-4 grid aspect-[16/10] place-items-center rounded-lg border border-dashed border-[#cfd3dc] bg-white px-3 text-center text-[10px] font-medium text-[#777d89]">Current-site screenshot pending verified input</div>}<ul className="mt-5 grid gap-3">{proposal.comparison.currentPoints.map((point) => <li key={point} className="flex gap-3 text-[15px] leading-6 text-[#606575]"><Check muted />{point}</li>)}</ul></article><article className="rounded-xl border border-[#d8e6a2] bg-[#fcfff4] p-4 sm:p-5"><div className="flex items-center justify-between gap-3"><p className="flex items-center gap-2 text-sm font-medium"><span className="h-3 w-3 rounded-full bg-[#d7ff00]" />New website concept</p><span className="text-[10px] font-medium uppercase tracking-[0.12em] text-[#758300]">Private concept</span></div><Image src={proposal.comparison.proposedImage.src} alt={proposal.comparison.proposedImage.alt} width={1440} height={960} className="mt-4 aspect-[16/10] w-full rounded-lg border border-[#dfe2e9] object-cover object-top shadow-sm" /><ul className="mt-5 grid gap-3">{proposal.comparison.proposedPoints.map((point) => <li key={point} className="flex gap-3 text-[15px] leading-6 text-[#424657]"><Check />{point}</li>)}</ul></article></div></section> : null}

          {proposal.conceptImages.supporting?.length ? <section className="rounded-xl border border-[#e7e9ef] bg-white p-5 sm:p-6" onMouseEnter={() => track(proposal, "concept_gallery_viewed")}><div className="flex flex-col gap-1 sm:flex-row sm:items-end sm:justify-between"><div><p className="text-[11px] font-semibold uppercase tracking-[0.18em] text-[#758300]">Concept views</p><h2 className="proposal-section-title mt-1">Designed for useful project enquiries</h2></div><p className="text-sm text-[#606575]">Private concept views, not a public website.</p></div><div className="mt-4 grid gap-4 sm:grid-cols-2">{proposal.conceptImages.supporting.map((image) => <figure key={image.src} className="overflow-hidden rounded-lg border border-[#dfe2e9] bg-[#f7f8fa]"><Image src={image.src} alt={image.alt} width={1440} height={960} className="h-auto w-full" /></figure>)}</div></section> : null}

          <div className="grid gap-5 lg:grid-cols-[0.72fr_1.28fr]">
            <section className="rounded-xl border border-[#e7e9ef] bg-white p-5 sm:p-6"><h2 className="proposal-card-title">Three opportunities we noticed</h2><div className="mt-5 grid gap-5">{proposal.findings.map((finding, index) => <article key={finding.title} className="flex items-start gap-4"><IconTile><Image src={opportunityIcons[index]} alt="" width={22} height={22} className="h-5 w-5 object-contain" /></IconTile><div><h3 className="text-[15px] font-medium leading-5">{finding.title}</h3><p className="mt-1 text-[15px] leading-6 text-[#606575]">{finding.description}</p></div></article>)}</div></section>
            <section className="rounded-xl border border-[#e7e9ef] bg-white p-5 sm:p-6"><h2 className="proposal-card-title">What this concept improves</h2><div className="mt-5 grid overflow-hidden rounded-lg border border-[#e7e9ef] sm:grid-cols-3">{proposal.improvements.map((improvement, index) => <article key={improvement.title} className="flex min-h-28 gap-4 border-b border-[#e7e9ef] p-4 sm:border-r"><IconTile><Image src={improvementIcons[index]} alt="" width={22} height={22} className="h-5 w-5 object-contain" /></IconTile><div><h3 className="text-[15px] font-medium leading-5">{improvement.title}</h3><p className="mt-1 text-[15px] leading-6 text-[#606575]">{improvement.description}</p></div></article>)}</div></section>
          </div>
          ...
          <section id="package" className="rounded-xl border border-[#e1e4ea] bg-white p-6 shadow-[0_10px_32px_rgba(24,31,51,0.05)] sm:p-8" onMouseEnter={() => track(proposal, "offer_viewed")}>
            <div className="proposal-package-grid gap-0 overflow-hidden">
              <div className="py-2 md:pr-7 lg:pr-8"><p className="text-[11px] font-semibold uppercase tracking-[0.18em] text-[#758300]">Website launch package</p><h2 className="proposal-package-heading mt-3">Launch this website for <span className="proposal-package-price">{proposal.package.priceLabel}</span></h2><p className="mt-1 text-sm font-medium text-[#777d89]">includes VAT/GST</p><div className="mt-5 h-1 w-8 rounded-full bg-[#d7ff00]" /><div className="proposal-body mt-5 grid gap-3"><p>The working website concept shown in your walkthrough has already been built for <strong className="font-medium text-[#0a0b10]">{companyName}</strong>.</p><p>If you decide to proceed, we will apply the agreed revisions, connect your domain and enquiry form, complete the final checks and prepare it for launch.</p></div></div>
              ...
              <div className="grid content-center gap-4 py-6 md:pl-7 md:py-2 lg:pl-9"><Button onClick={openLaunch} className="min-h-20 !justify-between !rounded-xl !px-6 !py-5 !text-left !text-base"><span>I&apos;d like to launch this website</span><Arrow /></Button><Button href={proposal.actions.calendlyUrl} secondary onClick={() => track(proposal, "call_requested")} className="min-h-20 !justify-between !rounded-xl !px-6 !py-5 !text-left !text-base">Book a free {callMinutes}-minute call <Arrow /></Button>{proposal.actions.interactivePreviewRequestUrl ? <Button href={proposal.actions.interactivePreviewRequestUrl} secondary onClick={() => track(proposal, "interactive_preview_requested")}>Request an interactive preview</Button> : null}<div className="mt-2 border-t border-[#e1e4ea] pt-5"><p className="flex items-center gap-4 text-[15px] leading-6 text-[#707582]"><span className="flex h-9 w-9 shrink-0 items-center justify-center [&>svg]:h-6 [&>svg]:w-6"><Lock /></span><span>No payment is taken by clicking. We will confirm the final scope and terms with you first.</span></p></div></div>
            </div>
          </section>
          ...
          <section className="flex flex-col gap-5 rounded-xl bg-gradient-to-r from-[#08090c] to-[#1c2027] p-6 text-white sm:flex-row sm:items-center sm:justify-between"><div><p className="text-[11px] font-semibold uppercase tracking-[0.18em] text-[#d7ff00]">Your next step</p><h2 className="mt-2 text-[clamp(1.7rem,3vw,2.25rem)] font-medium leading-[1.12]">Ready to use this concept for {proposal.companyName}?</h2><p className="mt-3 text-base leading-7 text-white/68">We will apply the agreed revisions, connect the domain and enquiry form, complete the final checks and prepare the website for launch.</p></div><div className="grid shrink-0 gap-2"><Button onClick={openLaunch} className="!bg-white !from-white !to-white !text-[#0a0b10]">I&apos;d like to launch this website <Arrow /></Button><button type="button" className="text-sm font-medium text-white/60 underline" onClick={() => setDialog("remove")}>Close my private proposal</button></div></section>
        </main>
      ...
      {dialog === "launch" ? <Dialog title="Ready to launch this website?" onClose={() => setDialog(null)}><p className="proposal-body">No payment is taken by clicking. Choose the easiest way to tell T3 Labs you would like to move forward, and we will confirm the final scope and terms with you first.</p><div className="mt-6 grid gap-3"><Button href={emailHref} onClick={() => track(proposal, "launch_email_requested")}>Email T3 Labs about launch <Arrow /></Button><Button href={proposal.actions.calendlyUrl} secondary onClick={() => track(proposal, "call_requested")}>Book a free {callMinutes}-minute call <Arrow /></Button></div></Dialog> : null}
