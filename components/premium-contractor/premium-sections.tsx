...
function ServicesPreview({ site }: { site: PremiumContractorConfig }) {
  ...
  return <section id="services" className={`${styles.section} ${styles.darkSection}`}><div className={styles.container}><SectionHeading eyebrow={content.eyebrow} title={content.heading} copy={content.description} /><div className={styles.servicePreviewGrid}>{site.services.map((service, index) => <article key={service.id} className={styles.servicePreview}><div className={styles.servicePreviewImage}>{service.image && <Image src={service.image.src} alt={service.image.alt} fill sizes="(max-width: 760px) 100vw, 25vw" />}</div><div className={styles.servicePreviewCopy}><span>{String(index + 1).padStart(2, "0")}</span><h3>{service.name}</h3><p>{service.shortDescription}</p></div></article>)}</div></div></section>;
}
